...
- **Live:** Dennis `https://dd-ocarina.netlify.app/`, Quest Master `https://dd-ocarina.netlify.app/admin.html`. Hosting auf Netlify, Projekt `dd-ocarina`. `netlify.toml` setzt den Veröffentlichungsordner auf `app`. Netlify ist mit diesem Repo verknüpft (bestätigt am 24.09.): Jeder Push auf `main` ist nach wenigen Sekunden live.
- **Entwurf live (24.09.):** `https://dd-ocarina.netlify.app/entwurf.html` zeigt Dennis' neues Menü mit dem echten Spielstand (liest nur, schreibt nie), `…/entwurf.html?demo` mit Beispielstand und Demo-Knöpfen. Die Hauptadresse zeigt weiter die alte App.
...
- **Umbau Optik (24.09.), im Entwurf gebaut:** `07-menue-umbau.md` bringt das Menü auf das Niveau des Startbildschirms. Der Nutzer hat entschieden: Richtung „neues Zelda", Seiten gleiten, Fee als Cursor, Seitenleiste oben statt Z und R. Gebaut in `app/entwurf.*`: Titelbild unscharf als Hintergrund (`assets/menue-wald.webp`), dunkle Glasplatten mit Gold aus dem Logo, Hylia Serif mit ergänzten Umlauten für alle Titel, Kamerafahrt nach PRESS START, Medaillons als Münzen, Items neu gezeichnet, Ergebnis als Vollbild-Moment mit fliegenden Packs. Geprüft mit Chromium in vier Größen und 14 Spielständen (07 Abschnitt 12). **Bilder vom Nutzer kommen nachträglich** (Dennis als Link von vorn für die Ausrüstung, Karte als Gemälde, 07 Abschnitt 8). Noch nicht auf einem echten iPhone getestet.
...
| `07-menue-umbau.md` | **Umbau Optik** (gebaut im Entwurf): Menü auf das Niveau des Startbildschirms, Farben aus dem Titelbild, Bausteine, Übergang, Bilder vom Nutzer (Abschnitt 8), Stand in Abschnitt 12 |
| `app/assets/intro-titel.webp`, `intro-fee.png`, `menue-wald.webp`, `hylia-serif.woff2`, `hylian-symbols.woff2`, `SCHRIFTEN.md` | Titelbild des Startbildschirms vom Nutzer (Fee herausgelöst, schwebt separat, im Menü ist sie der Cursor), unscharfe Fassung als Menü-Hintergrund und die Zelda-Schriften von zeldauniverse.net: Hylia Serif (Umlaute ergänzt) für PRESS START und alle Titel, Hylian Symbols als Quelle für die Medaillon-Zeichen. Fan-Schriften, nur privat und nicht kommerziell, Details in `SCHRIFTEN.md` |
| `app/entwurf.html`, `entwurf.css`, `entwurf.js` | Klickbarer Entwurf zu 06, Optik nach 07. Nutzt `config.js`, `engine.js`, `store.js` unverändert. Neue Texte, Farben und Kartenpunkte stehen oben in `entwurf.js` und wandern beim Bauen nach `config.js` |
...
0b. Umbau Optik: Bilder des Nutzers einbauen, sobald sie da sind (`07-menue-umbau.md` Abschnitt 8), Entwurf auf echtem iPhone prüfen.
